...
bool Astar::findPath(std::pair<int,int> startCellIndex, std::pair<int, int> targetCellIndex, std::function<float(const std::pair<int, int>&, const std::pair<int, int>&)> h, CELL_TYPE* board, std::pair<int, int> boardSize, std::vector<std::pair<int, int>*> outPath) {
    ...
    std::unordered_set<std::pair<int, int>, hash_int_pair> closedSet;
    //cameFrom[Node] represents the node immediately preceding it on the cheapest path from start
    // to node currently known.
    std::unordered_map<std::pair<int, int>, std::pair<int, int>,hash_int_pair> cameFrom;
    // For node n, gScore[n] is the cost of the cheapest path from start to n currently known.
    std::unordered_map<std::pair<int, int>, float, hash_int_pair> gScore;
    ...
    while (!(openQueue.size()>0)) {
        ...
        for (std::pair<int, int>* neighbor : neighbors) {
            ...
            std::unordered_map<std::pair<int, int>, float,hash_int_pair>::iterator neighbor_gScoreItr = gScore.find(*neighbor);
            ...
            if (tentative_gScore < neighbor_gScore) {
                ...
                std::pair<std::unordered_map<std::pair<int, int>, std::pair<int, int>,hash_int_pair>::iterator,bool> insertCameFromInfo= cameFrom.insert(std::pair<std::pair<int, int>, std::pair<int, int>>(neighborCopy.cellIndex, current.cellIndex));
                if (!insertCameFromInfo.second)
                    insertCameFromInfo.first->second = current.cellIndex;

                // gScore[neighbor] : = tentative_gScore
                std::pair<std::unordered_map<std::pair<int, int>, float,hash_int_pair>::iterator, bool> insertGScoreInfo = gScore.insert(std::pair<std::pair<int, int>, float>(neighborCopy.cellIndex, tentative_gScore));
